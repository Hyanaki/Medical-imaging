...
int main()
{
    ...
    double amplitudeTrans = 0.005; //meter.s^-1
    double amplitudeRot = 5; //deg.s^-1
    double period_motion= 5; //(second)
    ...
    for (int i = 0; i<ROI_height; i++){
    	for (int j = 0; j<ROI_width; j++){
    		DesiredFeatures[i*ROI_width+j]=ImageD[Hmin+i][Wmin+j];
    	}
    }
    ...
    vpImage<double> dI0dx(height, width,0); // x gradient components of the 2D image
    vpImage<double> dI0dy(height, width,0); // y gradient components of the 2D image
    vpImage<double> dI0dz(height, width,0); // z gradient components of the 2D image
    ...
    vpImage<unsigned char> gradx(height, width);
    vpImage<unsigned char> grady(height, width);
    vpImage<unsigned char> gradz(height, width);
    vpImageConvert::convert(dI0dx,gradx);
    vpImageIo::writePNG(gradx,"./gradien_x");
    vpImageConvert::convert(dI0dy,grady);
    vpImageIo::writePNG(grady,"./gradien_y");
    vpImageConvert::convert(dI0dz,gradz);
    vpImageIo::writePNG(gradz,"./gradien_z");
    ...
    try  {
        while(1)
        {
            ...
            for (int i = 0; i<ROI_height; i++){
                for (int j = 0; j<ROI_width; j++){
                    CurrentFeatures[i*ROI_width+j]=Image[Hmin+i][Wmin+j];
                }
            }
            ...
            probe_velocity = -0.4 * Lspi * VisualFeatureError; // Compute the probe velocity
#endif
            ...
            object_velocity[0] = amplitudeTrans * sin(2*M_PI/period_motion*time);
            object_velocity[1] = amplitudeTrans * sin(2*M_PI/period_motion*time);
            object_velocity[2] = amplitudeTrans * sin(2*M_PI/period_motion*time);
            object_velocity[3] = vpMath::rad(amplitudeRot) * sin(2*M_PI/period_motion*time);
			object_velocity[4] = vpMath::rad(amplitudeRot) * sin(2*M_PI/period_motion*time);
			object_velocity[5] = vpMath::rad(amplitudeRot) * sin(2*M_PI/period_motion*time);
